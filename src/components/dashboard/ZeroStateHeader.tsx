...
export const ZeroStateHeader: React.FC<ZeroStateHeaderProps> = ({
  ...
  const navItems = [
    { id: 'feed', label: 'Work Journal Feed' },
    { id: 'hub', label: 'Retention Hub' },
    { id: 'analytics', label: 'Analytics & Decay' },
    { id: 'spec', label: 'Architecture Spec' },
  ];

  const handleNavClick = (id: string) => {
    if (id === 'spec' && onToggleBlueprint) {
      onToggleBlueprint();
    }
    if (onSelectNav) {
      onSelectNav(id);
  ...
  const userInitial = (profile?.displayName || user?.email || 'U')[0].toUpperCase();
    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-[#E5E0D8] transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4 sm:gap-6">
          {/* Brand Identity */}
          <div className="flex items-center gap-2.5 group cursor-pointer" onClick={() => handleNavClick('feed')}>
              ...
              <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl border border-[#E5E0D8] shadow-lg py-2 z-50 text-left animate-in fade-in slide-in-from-top-2 duration-150">
                <div className="px-4 py-2 border-b border-slate-100">
                  <div className="text-xs font-semibold text-[#1E293B] truncate">
                    {profile?.displayName || user?.displayName || 'Data Science Engineer'}
                  </div>
