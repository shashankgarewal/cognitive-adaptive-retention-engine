  ...
  return (
    <section id="hero-section" className="relative pt-6 sm:pt-8 pb-6 sm:pb-8 px-4 sm:px-6 lg:px-8 text-center">
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        {/* Auth Card or Active Session Card */}
        {user ? (
          <div
            id="auth-card"
            className="mt-5 sm:mt-6 max-w-md mx-auto w-full bg-white rounded-2xl border border-emerald-200 shadow-sm p-6 text-left transition-all space-y-4"
          >
            ...
            <div className="flex items-center gap-3.5 bg-[#FAF8F5] p-3.5 rounded-xl border border-[#E5E0D8]">
              <div className="w-10 h-10 rounded-full bg-[#006948] text-white flex items-center justify-center font-bold text-base shadow-xs shrink-0">
                {user.displayName ? user.displayName.charAt(0).toUpperCase() : user.email?.charAt(0).toUpperCase() || 'E'}
              </div>
              <div className="min-w-0 flex-1">
                <div className="font-sans font-bold text-sm text-[#1E293B] truncate">
                  {user.displayName || 'Data Science Engineer'}
                </div>
                <div className="font-mono text-xs text-[#505F76] truncate">
                  {user.email}
                </div>
            ...
            <button
              ...
              className="w-full py-2.5 px-4 rounded-xl bg-[#006948] hover:bg-[#005439] text-white font-medium text-xs sm:text-sm flex items-center justify-center gap-2 transition-colors shadow-xs cursor-pointer"
            >
              <span>Launch Work Journal Feed</span>
              <ArrowRight className="w-4 h-4" />
            </button>

            {/* Security Footnote */}
            <div className="pt-2 border-t border-slate-100 flex items-center justify-center gap-1.5 text-[11px] font-mono text-slate-500 text-center">
              <Lock className="w-3 h-3 text-emerald-600 shrink-0" />
              <span>Private sandbox • Continuous Mastery Engine Active</span>
            </div>
          </div>
        ) : (
          <div
