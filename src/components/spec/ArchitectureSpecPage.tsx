...
import { SystemBlueprintSpecSection } from './SystemBlueprintSpecSection';
...
export const ArchitectureSpecPage: React.FC<ArchitectureSpecPageProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] text-[#1E293B] font-sans flex flex-col selection:bg-emerald-500/20 selection:text-emerald-900 antialiased">
      {/* 1. Global Navigation Navbar (Consistently styled matching Landing Header) */}
      <AppNavbar
        activeNav="spec"
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#ECFDF5] border border-emerald-300 text-[#006948] font-mono text-xs font-semibold shadow-2xs">
                ...
                PROJECT SCOPE: care-retention-engine
              </span>
              <span className="hidden sm:inline text-xs font-mono text-[#505F76]">
                GCP: us-central1 • Vertex AI Gemini 3.8 Flash &amp; Cloud Run Active
              </span>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 flex-1 w-full">
        {/* System Architecture Blueprint (Consolidated from zero-state blueprint) */}
        <section aria-labelledby="system-blueprint-section">
          <SystemBlueprintSpecSection />
        </section>

        {/* Pipeline Diagram Card (Sequential Ingestion Nodes) */}
        <section aria-labelledby="pipeline-section">
          <PipelineDiagramCard />
        </section>

        {/* 2x2 Deep-Dive Specification Grid */}
        <section aria-labelledby="spec-grid-section">
          <DeepDiveSpecGrid />
        </section>

        {/* Interactive Telemetry Test Harness Inset */}
        <section aria-labelledby="test-harness-section">
          <TelemetryTestHarness />
        </section>
      </main>

      {/* Full-width Unified Footer */}
      <AppFooter />
    </div>
  );
};
